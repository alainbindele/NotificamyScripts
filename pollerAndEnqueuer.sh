...
build_json_message() {
    ...
    local json="{\"query_id\": $query_id, \"user_email\": \"$user_email\", \"prompt\": \"$prompt\""
    
    # Add optional fields only if they are not null/empty
...
while IFS= read -r line; do
    ...
    if [[ "$id" =~ ^[0-9]+$ && -n "$prompt" && -n "$user_email" ]]; then
        ...
        message_body=$(build_json_message "$id" "$user_email" "$prompt" "$user_discord_webhook" "$user_slack_webhook" "$user_phone")
